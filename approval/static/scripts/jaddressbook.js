...
var addressbookClass = function(){
	...
	this._useMenu = "";
	this._orderKey = getCookie("orderKey");
	this._orderValue = getCookie("orderValue");
};
	...
	} catch(E)
	{
		...
		click_menuTab : function()
		{
			...
			switch(Tab)
			{
				...
				case CONSTRUCT_TAB :
					var param = {"pTab" : Tab, "pFrom" : addressbook._from, "pTarget" : "ALL", "pValue" : "", "pKeyword" : "", "pField" : "", "pView" : addressbook._viewFile, "pOrderKey": this._orderKey, "pOrderValue": this._orderValue};
					if(addressbook._from == FROM_CALL){
	...
	*/
	} catch(E)
	{
		...
		click_orderBtn : function(pOrderKey, pOrderValue)
		{
			...
			setCookie("orderKey", pOrderKey, 365);
			setCookie("orderValue", pOrderValue, 365);
			...
			for(var i=0; i < options.length; i++) {
				...
				if(option.onclick.toString().includes(pOrderKey)) {
