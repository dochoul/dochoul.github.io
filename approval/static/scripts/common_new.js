...
var Common = {
    ...
    pagination: function (page, count_per_page, total_cnt, method) {
        var total_page = Math.ceil(total_cnt / count_per_page);
        var pageStr = "";
        if (page > 1) {
            pageStr += "<a onclick=\"" + method + ".MovePage('1');\"  style=\"cursor:pointer;\" ><span class=\"icon pagenav1\"></span></a>";
        }
        if (page > 10) {
            pageStr += "<a title=\"이전\"  style=\"cursor:pointer;\"  onclick=\"" + method + ".MovePage('" + parseInt(Math.ceil(page / 10) - 1) * 10 + "');\" ><span class=\"icon pagenav2\"></span></a>";
        }
        for (var i = (Math.ceil(page / 10) - 1) * 10 + 1; i <= total_page && i < Math.ceil(page / 10) * 10 + 1; i++) {
            if (page == i) {
                pageStr += "<span class=\"paging_numbers\"><strong>" + i + "</strong></span>";
            } else {
                pageStr += "<a  style=\"cursor:pointer;\"  onclick=\"" + method + ".MovePage('" + i + "');\"><span>" + i + "</span></a>";
            }
        }
        if (Math.ceil(total_page / 10) > Math.ceil(page / 10)) {
            pageStr += "<a  style=\"cursor:pointer;\"  onclick=\"" + method + ".MovePage('" + (parseInt(10 * Math.ceil(page / 10)) + 1) + "');\"  title=\"다음\"><span class=\"icon pagenav3\"></span></a>";
        }
        if (page < total_page) {
            pageStr += "<a title=\"끝\"  style=\"cursor:pointer;\"  onclick=\"" + method + ".MovePage('" + total_page + "');\" ><span class=\"icon pagenav4\"></span></a>";
        }
        return pageStr;
    },
	loading_on: function (txt) {
		if (txt == undefined) {
			txt = '';
		}
		var obj = $j('<div>').attr('id', 'popupBase');
        obj.html('<div id="m_write_upload_progress" class="hw-loading"><div class="img"><img src="/static/ui/images/hw-loading.gif" alt=""></div><div class="text" id="txt_hw_loading_txt" >' + txt + '</div></div>');
        $j(document.body).append(obj);
		showPopup($j('#popupBase'));
	},
...
var adminTop = {
    ...
    initMouse: function () {
        if (document.getElementById("adminButtonLI")) {
            document.getElementById("adminButtonLI").onmouseover = function () {
                adminTop.mouseFlag = true;
            }
            document.getElementById("adminButtonLI").onmouseout = function () {
                adminTop.mouseFlag = false;
            }
...
String.prototype.isEmail = function () {
    return (/\w+([-+.]\w+)*@\w+([-.]\w+)*\.[a-zA-Z]{2,63}$/).test(this.trim());
}
...
function setCookieWithDomain(c_domain, c_name, value, expiredays) {
    var exdate = new Date();
    exdate.setDate(exdate.getDate() + expiredays);
    var cookiePath = '/';
    document.cookie = c_name + "=" + escape(value) + ((expiredays == null) ? "" : ";expires=" + exdate.toGMTString()) + ";path=" + cookiePath + ';domain=' + c_domain;
}

function setTodayCookie(c_name, value) {
    var date = new Date();
    var expires = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59));
    var cookiePath = '/';
    document.cookie = c_name + "=" + escape(value) + (";expires=" + expires.toGMTString()) + ";path=" + cookiePath;
}
